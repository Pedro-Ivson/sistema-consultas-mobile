...
export default function Agendar({ usuario, navigation }: AgendarProps) {
  ...
  function escolherData(dia: Date) {
    ...
    setErros((prev) => ({
      ...prev,
      data: validarDataAgenda(dia) ?? undefined,
    }));
    setSucesso("");
  }
      ...
      <ScrollView contentContainerStyle={styles.conteudo}>
        ...
        <View style={[styles.secao, erros.medico && styles.secaoErro]}>
          ...
          {!especialidadeId ? (
            ...
            <Text style={styles.texto}>
              Nenhum médico ativo nesta especialidade.
            </Text>
          ) : (
            <ListaSelecao
          ...
          {erros.medico ? (
            <Text style={styles.textoErro}>{erros.medico}</Text>
          ) : null}
        </View>
        ...
        <View style={[styles.secao, erros.data && styles.secaoErro]}>
          ...
          <Text style={styles.texto}>
            Não agenda hoje nem o passado. O último dia habilitado é daqui a 2
            meses.
          </Text>
          <SeletorData selecionada={data} onSelecionar={escolherData} />
          {erros.data ? (
            <Text style={styles.textoErro}>{erros.data}</Text>
          ) : null}
        </View>
