...
export default function Admin({
  ...
  function adicionarEspecialidade() {
    if (!nomeEsp || !descEsp) {
      mostrarAlerta("Erro", "Preencha nome e descrição");
      return;
    }
    ...
    setNomeEsp("");
    setDescEsp("");
    mostrarAlerta("Sucesso", "Especialidade adicionada!");
  }
  ...
  function adicionarMedico() {
    ...
    if (especialidades.length === 0) {
      mostrarAlerta("Erro", "Adicione uma especialidade primeiro!");
      return;
    }
  ...
  async function criarConsultaTeste() {
    if (!nomePac || !dataConsulta) {
      mostrarAlerta("Erro", "Preencha nome do paciente e data");
      return;
    }

    if (medicos.length === 0) {
      mostrarAlerta("Erro", "Adicione um médico primeiro!");
      return;
    }
    ...
    if (Platform.OS === "web") {
      mostrarAlerta("Sucesso", "Consulta criada! Volte para Home");
